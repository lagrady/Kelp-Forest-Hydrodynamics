...
def runMean(data, length, dt=1, keepnans=True):
    ...
    ret[0:int(np.ceil(length/2))] = temp[int(np.floor(length/2)):length]#temp[int(np.floor(length/2))-2:length]
    ret[int(-np.floor(length/2)):] = temp[-1] - temp[-(length):-int(np.ceil(length/2))] #ret[int(-np.floor(length/2))+2:]
    ...
    # find the sums of the end points
    counts[0:int(np.ceil(length/2))] = temp[int(np.floor(length/2)):length] #temp[int(np.floor(length/2))-2:length]
    counts[int(-np.floor(length/2)):] = temp[-1] - temp[-(length):-int(np.ceil(length/2))] #counts[int(-np.floor(length/2))+2:]
...
#region avTime30(vector)
def avTime20(vector):
    #create a 20 minute burst timestamp and burst numbers
    ...
    #set a 10 minute stime step (half of total burst time)
    dt = np.timedelta64(10,'m')

    #create new 20 minute time stamp starting 10 minutes after the first time value
    tav = np.arange(time[0].astype('datetime64[m]') + dt, time[-1].astype('datetime64[m]'),2*dt)

    #initialize a burst number array
    bNum20 = np.empty(time.size)*np.nan

    #create a variable with the burst number for every data point (on the original time stamp)
    for i in np.arange(tav.size):
        bNum20[np.logical_and(time>=tav[i]-dt,time<tav[i]+dt)] = i
    
    #save new time stamp and burst number variables
    vnew.coords['avTime20'] = tav
    vnew['bNum20'] = ('time',bNum20)
    vnew['burst20'] = ('avTime20', np.arange(tav.size))
...
def ProcessVec(data,badSections,reverse,expand = True,lp = 1/20,expSize = 0.01,expEnd = 0.95):
    ...
    data_temp_All = vt.badDataRatio(data_temp_All, EOrig, 'dEast')
    data_temp_All = vt.badDataRatio(data_temp_All, NOrig, 'dNorth')
    data_temp_All = vt.badDataRatio(data_temp_All, UpOrig, 'dUp')
    data_temp_All = vt.badDataRatio(data_temp_All, PrimOrig, 'dPrimary')
    data_temp_All = vt.badDataRatio(data_temp_All, SecOrig, 'dSecondary')
...
def specSegmentNorm(data,seglength,segnum):
    ...
    if np.remainder(ltotal,seglength) == 0:
        segments = np.concatenate((np.reshape(data,(int((segnum+1)/2),int(seglength))),\
            np.reshape(data[int(seglength/2):int(np.floor(-seglength/2))],\
                (int((segnum-1)/2),int(seglength)))),0)
    else:
        segments = np.concatenate((np.reshape(data[0:-int(np.remainder(ltotal,seglength))],(int((segnum+1)/2),int(seglength))),\
            np.reshape(data[int(seglength/2):int(np.floor(-np.remainder(ltotal,seglength)\
                -seglength/2))],(int((segnum-1)/2),int(seglength)))),0)
...
def specSegment(data,seglength):
    ...
    for d in data:
        ...
        if np.remainder(ltotal,seglength) == 0:
            segments = segments + (np.reshape(d,(int(segnum),int(seglength))),)
        else:
            segments = segments + (np.reshape(d[0:-int(np.remainder(ltotal,seglength))],(int(segnum),int(seglength))),)
...
def epCalc(vec,name,segSize = 10, highLowSep = 1/5, turbVarCutoff = 5, waveCutoff = 1.025, unorigionalCutoff = 0.01,\
    ...
    bnum = vec.BurstCounter.values
    hz = vec.attrs['Sampling_Rate']

    #pull time of each burst
    t = vec.time_start.values
    ...
    tempAll = vec.Temperature.interp(time_sen=vec.time)
    ...
    for i in np.arange(bnum.size):
        ...
        ind = vec.BurstNum.values == bnum[i]
